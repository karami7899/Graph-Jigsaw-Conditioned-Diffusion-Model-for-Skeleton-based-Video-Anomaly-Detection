...
from graph.graph_dataset import make_graph
...
class GiCiSAD(pl.LightningModule):
    ...
    def __init__(self, args:argparse.Namespace) -> None:
        ...
        self.graph_input_dim=args.graph_input_dim
        self.graph_out_layer_num=args.graph_out_layer_num
        self.graph_out_layer_inter_dim=args.graph_out_layer_inter_dim
    ...
    def build_model(self) -> None:
        ...
        if self.conditioning_strategy == 'inject':
            if self.conditioning_architecture == 'Graph':
              feature_map = get_feature_map(2*17)
              fc_struc = get_fc_graph_struc(2*17)
              fc_edge_index = build_loc_net(fc_struc, feature_map, feature_map=feature_map)
              fc_edge_index = torch.tensor(fc_edge_index, dtype = torch.long)
              edge_index_sets = []
              edge_index_sets.append(fc_edge_index)
              condition_encoder = GDN(edge_index_sets, len(feature_map), 
                dim=self.graph_dim, 
                input_dim=self.graph_input_dim,
                out_layer_num=self.graph_out_layer_num,
                out_layer_inter_dim=self.graph_out_layer_inter_dim,
                topk=self.graph_topk,
                Mydevice = self.device).to(self.device)

            elif self.conditioning_architecture == 'AE':
    ...
    def forward(self, input_data:List[torch.Tensor], aggr_strategy:str=None, return_:str=None) -> List[torch.Tensor]:
        ...
        #
        reduced_corrupt_data= torch.mean(corrupt_data, dim=2)
        result_avg_expanded = reduced_corrupt_data.unsqueeze(2)  # Adds a third dimension

        # Concatenate along the third dimension (dim=2)
        expand_condition_data = torch.cat((condition_data, result_avg_expanded), dim=2)
       
        condition_data_graph = make_graph(expand_condition_data)
        for x, labels, attack_labels, edge_index in condition_data_graph:
            x, labels, edge_index = [item.float().to(self.device) for item in [x, labels, edge_index]]
            condition_embedding, rec_cond_data, arrangement_predictions, arrangement_labels = self.condition_encoder(x, edge_index)
            rec_cond_data = rec_cond_data.float().to(self.device)
    ...
    def training_step(self, batch:List[torch.Tensor], batch_idx:int) -> torch.float32:
        ...
        elif self.conditioning_architecture == 'Graph':
          loss_rec_cond = F.mse_loss(rec_cond_data, labels)
          arrangement_loss = F.cross_entropy(arrangement_predictions, arrangement_labels)
          loss = loss_noise + (loss_rec_cond + arrangement_loss * self.weight1) * self.weight2
          self.log("loss_recons", loss_rec_cond)
